...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		$data['sort_login'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=login' . $url, true);
	...
	protected function getForm() {
		...
		$this->load->model('customer/review');

		$data['reviews'] = [];

		$reviews = $this->model_customer_review->getReviews($this->request->get['customer_id']);
        ...
        foreach ($reviews as $review) {
            $data['reviews'][] = [
                ...
                'time' => $review['time'],
                'date_added' => format_date($review['date_added'], 'Y.m.d H:i'),
                'customer_login' => $review['customer_login'],
                'customer_href' => $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $review['customer_id'], true)
            ];
        }
	...
	public function transaction() {
		...
		foreach ($results as $result) {
			$data['transactions'][] = array(
				...
				'date_added'  => format_date($result['date_added'], 'd.m.Y H:i:s')
			);
		}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_login']) || isset($this->request->get['filter_email'])) {
			if (isset($this->request->get['filter_login'])) {
				$filter_login = $this->request->get['filter_login'];
			} else {
				$filter_login = '';
