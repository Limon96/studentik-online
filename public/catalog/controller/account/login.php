...
class ControllerAccountLogin extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($this->request->get['act'])) {
            $data['act'] = $this->request->get['act'];
        } else {
            $data['act'] = 'login';
        }


        $data['action_login'] = $this->url->link('account/login/login', '', true);
    ...
    protected function validate()
    {
        ...
        $login_info = $this->model_account_customer->getLoginAttempts($this->request->post['login']);
        ...
        if (!$this->error) {
            if (!$this->customer->login($this->request->post['login'], $this->request->post['password'])) {
                $this->error['warning'] = $this->language->get('error_login');

                $this->model_account_customer->addLoginAttempt($this->request->post['login']);
            } else {
                $this->model_account_customer->deleteLoginAttempts($this->request->post['login']);
            }
        }
    ...
    {
        $this->load->language('account/login');
        // если не авторизован - авторизируем
        if (!$this->customer->isLogged()) {
            $this->load->model('account/customer');
            ...
            if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
                ...
                $json['redirect'] = $this->url->link('order/order');
