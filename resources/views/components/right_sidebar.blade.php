<div id="column-right" class="right_content_resp">
    <button class="open_right_colmn">
        <svg class="op" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 330 330" style="enable-background:new 0 0 330 330;" xml:space="preserve">
<g>
    <g>
        <g>
            <path d="M165,0.008C74.019,0.008,0,74.024,0,164.999c0,90.977,74.019,164.992,165,164.992s165-74.015,165-164.992     C330,74.024,255.981,0.008,165,0.008z M165,299.992c-74.439,0-135-60.557-135-134.992S90.561,30.008,165,30.008     s135,60.557,135,134.991C300,239.436,239.439,299.992,165,299.992z"></path>
            <path d="M165,130.008c-8.284,0-15,6.716-15,15v99.983c0,8.284,6.716,15,15,15s15-6.716,15-15v-99.983     C180,136.725,173.284,130.008,165,130.008z"></path>
            <path d="M165,70.011c-3.95,0-7.811,1.6-10.61,4.39c-2.79,2.79-4.39,6.66-4.39,10.61s1.6,7.81,4.39,10.61     c2.79,2.79,6.66,4.39,10.61,4.39s7.81-1.6,10.609-4.39c2.79-2.8,4.391-6.66,4.391-10.61s-1.601-7.82-4.391-10.61     C172.81,71.61,168.95,70.011,165,70.011z"></path>
        </g>
    </g>
</g>
</svg>
        <svg class="cl" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 492 492" style="enable-background:new 0 0 492 492;" xml:space="preserve"><path d="M300.188,246L484.14,62.04c5.06-5.064,7.852-11.82,7.86-19.024c0-7.208-2.792-13.972-7.86-19.028L468.02,7.872    c-5.068-5.076-11.824-7.856-19.036-7.856c-7.2,0-13.956,2.78-19.024,7.856L246.008,191.82L62.048,7.872    c-5.06-5.076-11.82-7.856-19.028-7.856c-7.2,0-13.96,2.78-19.02,7.856L7.872,23.988c-10.496,10.496-10.496,27.568,0,38.052    L191.828,246L7.872,429.952c-5.064,5.072-7.852,11.828-7.852,19.032c0,7.204,2.788,13.96,7.852,19.028l16.124,16.116    c5.06,5.072,11.824,7.856,19.02,7.856c7.208,0,13.968-2.784,19.028-7.856l183.96-183.952l183.952,183.952    c5.068,5.072,11.824,7.856,19.024,7.856h0.008c7.204,0,13.96-2.784,19.028-7.856l16.12-16.116    c5.06-5.064,7.852-11.824,7.852-19.028c0-7.204-2.792-13.96-7.852-19.028L300.188,246z"></path></svg>

    </button>


    <div class="clearfix worker_right">
        <h3>Топ экспертов за неделю</h3>
        <div class="item_worker clearfix">
            <div class="img_face clearfix">
                <a href="https://studentik.online/user/georgy"><img src="https://studentik.online/image/cache/profile-80x80.png" alt="Georgy"></a>
            </div>
            <div class="info_worker clearfix">
                <div class="lit_hed clearfix">
                    <a href="https://studentik.online/user/georgy">Georgy</a>
                </div>
                <div class="lit_foot clearfix">
                    <div class="enter clearfix">
                        <span class="name">Рейтинг:</span>
                        <span class="rating">1556</span>
                    </div>
                    <div class="enter_like clearfix">
                        <img src="../catalog/assets/img/icons/like.svg">
                        <span class="like">13</span>
                        <img src="../catalog/assets/img/icons/dislike.svg">
                        <span class="dislike">1</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="item_worker clearfix">
            <div class="img_face clearfix">
                <a href="https://studentik.online/user/proexpert"><img src="https://studentik.online/image/cache/profile-80x80.png" alt="ProExpert"></a>
            </div>
            <div class="info_worker clearfix">
                <div class="lit_hed clearfix">
                    <a href="https://studentik.online/user/proexpert">ProExpert</a>
                </div>
                <div class="lit_foot clearfix">
                    <div class="enter clearfix">
                        <span class="name">Рейтинг:</span>
                        <span class="rating">1325</span>
                    </div>
                    <div class="enter_like clearfix">
                        <img src="../catalog/assets/img/icons/like.svg">
                        <span class="like">20</span>
                        <img src="../catalog/assets/img/icons/dislike.svg">
                        <span class="dislike">0</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="item_worker clearfix">
            <div class="img_face clearfix">
                <a href="https://studentik.online/user/user13"><img src="https://studentik.online/image/cache/profile-80x80.png" alt="user13"></a>
            </div>
            <div class="info_worker clearfix">
                <div class="lit_hed clearfix">
                    <a href="https://studentik.online/user/user13">user13</a>
                </div>
                <div class="lit_foot clearfix">
                    <div class="enter clearfix">
                        <span class="name">Рейтинг:</span>
                        <span class="rating">723</span>
                    </div>
                    <div class="enter_like clearfix">
                        <img src="../catalog/assets/img/icons/like.svg">
                        <span class="like">18</span>
                        <img src="../catalog/assets/img/icons/dislike.svg">
                        <span class="dislike">0</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="foot_worker clearfix">
            <a href="https://studentik.online/experts">
                <img src="../catalog/assets/img/icons/eye.svg">
                <span>Смотреть все</span>
            </a>
        </div>
    </div>

    <div class="contact_right clearfix">
        <h3>Контакты</h3>
        <a href="mailto:studentik.online@mail.ru" class="mail">studentik.online@mail.ru</a>
        <a href="https://studentik.online/index.php?route=information/information&amp;information_id=5" class="polit">Пользовательское соглашение</a>    <a href="https://studentik.online/index.php?route=information/information&amp;information_id=3" class="polit">Политика конфиденциальности</a>    <h4>Мы в соцсетях</h4>
        <ul class="social clearfix">
            <li><a target="_blank" href="https://vk.com/studentik.online"><img src="../catalog/assets/img/icons/vk.svg"></a></li>
            <li><a href="tg://resolve?domain=studentik_online"><img src="../catalog/assets/img/icons/telegram.svg"></a></li>
            <li><a target="_blank" href="https://www.instagram.com/studentik.online/"><img src="../catalog/assets/img/icons/instg.svg"></a></li>
        </ul>










        <a href="https://studentik.online/contacts" class="btn_mod">Написать нам</a>


    </div>
</div>
